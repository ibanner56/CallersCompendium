...
- [x] 6.3 Import review-queue UI (adapter-agnostic; wired to `GenericJsonAdapter`) — in-app import experience in `app/lib/src/screens/import_review_screen.dart`: source input (`.json` file picker or paste JSON) → non-destructive `ImportPipeline.plan` → review queue (per-record parse-quality, issues, and dedupe verdict with new/reimport/ambiguous actions; ambiguous defaults to skip, never a silent create) → `commit` → result summary → `undo`, with a live Collection refresh (`CollectionRefreshScope`). Reached from Settings › General. Adapter-agnostic so CallersBox (6.2)/ContraDB (6.4)/CC (6.5) sources plug in without a rewrite. (Label previously read "CallersBox snapshot import in-app"; reclassified per the 6.1 note that "review-queue UI is 6.3".)
- [ ] 6.4 ContraDB import
