...
- [ ] G.2 **Require "mark performed" for calling history** — a General settings
  ...
  calling-history section (Phase 4 per-dance calling history; `docs/design/ux.md`
  §2). Default (off): a program appears in a dance's history as soon as it
  contains that dance, regardless of whether the slot was marked performed. When
  on: only programs with the dance's slot marked performed appear, matching the
  strict "actually called" reading of the derived history query
