...
- [ ] 6.3 CallersBox snapshot import in-app
  ...
  - **Free-text figures now structure where recognized** (shared-parser phase): CallersBox figure lines route through the new core `parseFigureLine` (`imports/figure_parser.dart`), which conservatively upgrades recognized moves (swing/balance/circle/star/chain/allemande/do-si-do/shoulder-round/…) into structured taxonomy figures and degrades everything else to the same dialect-scrubbed `custom` figure as before (parse-never-fails; source beats + section-label prefix on custom preserved). The previously-deferred `(beats) text` → taxonomy structuring for CallersBox is delivered here; box still left for the tech lead per the note above.
- [x] 6.4 ContraDB import
  ...
  - **Shared free-text parser landed (that follow-up phase).** The ContraDB-HTML figure path now routes its scrubbed `(section, beats, text)` tuples through the core `parseFigureLine` (`imports/figure_parser.dart`): recognized moves become structured taxonomy figures, the rest fall back to the identical `custom` figure (with the `'$label: $scrubbed'` prefix + progression flag preserved). Validated against the real `dances/1`-modeled "The Rendezvous" fixture (balance & swing, long lines, circle → structured; the "or"/multi-move lines correctly stay custom).
