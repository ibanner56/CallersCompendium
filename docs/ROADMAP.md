...
- **UI localization / multi-language** — the i18n **framework has landed** (see
  ...
  **English as the source locale**. UI-string extraction into `app_en.arb` is now
  **complete** across every screen, so the app is **translation-ready**; aside from a
  small set of core service-layer messages and exported-document body text (both
  deferred by design), the interface is fully externalized. It remains **English-only**
  for now, with no translations shipped. What remains is welcoming
