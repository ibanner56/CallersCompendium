...
| `deviceScoped` | Never transmitted **by any route at all**, because the value is meaningless or actively wrong on another device — a window position, a per-device marker. Distinct from `deviceLocal`: that is withheld for what it *contains*, this for what it *means*. A value a transport must carry to function, such as a device identifier, is **not** this class and does not get an exception from it; see `EgressClass.protocolIdentifier` in `data_classification.dart` |
