...
- **`stable.json`** / **`beta.json`** — the static update manifest
  ...
  client. Release codenames are included for the Pages landing page. It is
  shipped as a release asset **and published to GitHub Pages** so the client's
  fixed URL resolves — see
...
refreshes the same draft release. If the tag's release is already published
