...
**Every path that decides existence.** The comparison above is consulted from
several places, and a change to it must be traced to all of them:
...
The review-methodology entries that stood here have moved to
[design-review.md](../dev/design-review.md), under **Findings from the Device
Sync design review**. The two that follow are kept because they carry design
rationale rather than review method.
...
The remaining review-methodology entries that stood here have moved to
[design-review.md](../dev/design-review.md), under **Findings from the Device
Sync design review**.
...
| Bytes per store | 250 MB | ~15x a full Caller's Box import. Counts blobs and device manifests. |
...
`U+0065 U+0301` produced by the former NFC-first implementation. The repair
backfill is algorithm-versioned so rows written by the former implementation
are scanned again.
...
The entry **A grep that finds nothing is an exclusion, and needs its
population** has moved to [design-review.md](../dev/design-review.md), under
...
These have moved to [design-review.md](../dev/design-review.md), under **Review
habits this design produced**. They came out of reviewing this design rather
than out of the sync mechanism itself, and they generalise beyond Device Sync.
