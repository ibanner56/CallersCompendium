...
- v35 (issue #1233): replaces nullable `program_slots.planned_minutes` with
  nullable `walkthrough_minutes` and `dance_minutes`. The forward-only
  migration copies every legacy value, including explicit zero, to
  `dance_minutes`; existing walkthrough values are NULL. Program-slot timing
  is structured metadata and does not feed derived dance indexes, so no
