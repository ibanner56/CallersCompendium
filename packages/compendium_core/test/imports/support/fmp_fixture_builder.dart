/// A tiny, from-scratch encoder for the FileMaker 12 (`.fmp12`) container
/// structures the reader consumes, used *only* by tests.
///
...
/// chunks) needed to describe one table with columns and a couple of rows. This
/// lets the reader be exercised end-to-end **hermetically** (no real, licensed
/// FileMaker file in CI), complementing the byte-for-byte validation against
/// real files that runs locally.
///
/// It is not a general FileMaker writer and only produces the shapes the reader
/// reads back; it is intentionally in `test/` so it never counts toward the
/// core's `lib/` coverage or ships in the package.
library;
