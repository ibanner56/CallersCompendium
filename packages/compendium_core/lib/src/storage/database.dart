...
///   column (a 1..5 integer; `null` = unrated). Existing rows get NULL. Rating
///   is dance-scalar curation metadata, not figure text, so it does not feed
///   the derived `dance_figures`/`dance_fts` indexes and no derived rebuild is
///   required.
///
...
class CompendiumDatabase extends _$CompendiumDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) async {
      ...
      if (from < 6) {
        // CC-parity dance rating. Nullable (existing rows get NULL); a 1..5
        // integer (`null` = unrated). Dance-scalar curation metadata (not
        // figure text), so it doesn't feed the derived `dance_figures`/
        // `dance_fts` indexes and no rebuild is required.
