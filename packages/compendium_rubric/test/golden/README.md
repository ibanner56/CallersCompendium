...
| `last_hey.json` | The first golden containing a **hey**, and the dance that settled how `pass2` is read: it names the *second pass*, not the ends pair. One correction — the record called its hey a `half` where the source gives sixteen beats and seven named passes, so it carries `full`. That correction has since become unnecessary: `compendium_core` #1159 changed the import dialect so an unqualified `Hey (...)` reads as `full`. Because a full hey is a positional identity, its twin assertion substitutes a **half** hey and requires the dance to stop landing. |
| `the_carousel.json` | The first golden whose hey **opens on the side**, and the dance the reading was derived from. Its A2 `role2s` allemande leaves the partners on the same side of the set, so `pass1: partners` names a pass that cannot happen in the middle. Read as a centre opening the figure refuses, so **the compile is itself the assertion** — this dance did not compile before the side opening was modelled. Verbatim: the record already flags the B2 neighbour swing as the progression, so no correction was needed. Note that Last Hey's half-hey substitution **does not transfer** — a side-opening half hey is deferred, so shortening this one tests the deferral rather than the weave, and its test says so. |