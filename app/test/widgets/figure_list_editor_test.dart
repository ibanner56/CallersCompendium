...
void main() {
  ...
  group('promenade turn/destination visibility & reset (#989, v30)', () {
    testWidgets(
      'turn is hidden and reset to unspecified once dir leaves the '
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // That nudge was removed; only the manual toggle remains. Scope the
      // assertion to the toggle's own row so an unrelated info icon
      // elsewhere in the editor can't mask a regression here.
      final drafts = <FigureDraft>[FigureDraft()];
      await _pump(tester, drafts);
      await _selectMove(tester, 0, 'sw', 'swing');

      final progressionRow = find.byKey(
        const ValueKey('figure-0-progression-row'),
      );
      expect(
        find.descendant(
          of: progressionRow,
  ...
  group('unknown move (#358)', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final drafts = <FigureDraft>[
          FigureDraft.fromFigure(
            // invalid-fixture: move is deliberately outside the taxonomy — opens read-only without throwing and preserves the stored data
            Figure(
    ...
    testWidgets('reorder and delete remain available for an unknown move', (
      ...
      final drafts = <FigureDraft>[
        ...
        FigureDraft.fromFigure(
          // invalid-fixture: move is deliberately outside the taxonomy — reorder and delete remain available for an unknown move
