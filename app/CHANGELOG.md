...
- **Importing a `.ccshare` file through the manual Import picker now includes the program.** Previously, picking a `.ccshare` file from the "Choose file" button silently dropped any program it contained — you'd see the dances arrive but the program (with its event details, venue, and slot order) was quietly lost. Now the program, venue, and all slot details are imported alongside the dances, exactly as they are when you open the file directly from Files. The import can be undone in one tap.
