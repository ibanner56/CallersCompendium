...
- **`chain` now carries a `hand` param** (left/right), matching ContraDB's
  model. Importing a "left-hand ladies chain" (or ContraDB's rendered
  "ladles left-hand chain") no longer falls back to a custom figure; a hand
  that agrees with the chaining role's implicit side (ladies → right,
  gents → left) renders silently, exactly as before, while a stated hand
  that contradicts the role still renders, hyphenated. Existing chains
  stored before this release are updated in place on first launch so
  structured search (Advanced ▸ has figure ▸ chain ▸ hand) finds them
  too, regardless of when they were imported. (#976)
